...
- **Resultado de recurso FAP é o alerta de maior valor** e tem tratamento
  ...
  ganho de causa. A linha leva o **badge `FAP`** vermelho antes do título, e
  clicar nele filtra. Dois cuidados na cor: o par `--dou-fap`/`--dou-fap-ink`
  existe porque **um vermelho só não serve nos dois temas** — no escuro ele
  precisa clarear para não sumir no fundo, e aí texto branco cai para 3,8:1,
  abaixo do mínimo de 4,5 (fundo claro leva texto branco; vermelho-claro leva
  texto quase preto — medido em 5,62:1 e 5,95:1). E a regra é escopada em
  `.dou-page .dou-tag-fap` porque o app declara `[data-bs-theme="dark"] a` com
  especificidade (0,1,1), que vencia o `color` de uma classe só.
...
**Resposta cortada no limite de saída (não existe teto de raciocínio).** Nos modelos com thinking o raciocínio sai do **mesmo** orçamento de saída que o JSON; quando consome tudo, o JSON vem cortado no meio e a revisão morre. O `reasoning.max_tokens` do OpenRouter é **aceito e ignorado** pelo `anthropic/claude-sonnet-5` — medido em 09/09/2026 com orçamento pedido de 1.024: 2.699 e 3.813 do jeito que a produção mandava, 3.288 com `max_tokens` no campo que a documentação exige, 2.998 com o provedor fixado em `anthropic`. A documentação promete o contrário; **a medição vence**. O que o provedor honra é `effort` (`medium` → 182, `low` → 0, sem parâmetro → 2.785), que reduz mas não limita — o gasto acompanha a dificuldade da tarefa. Por isso a defesa é `_invocar_com_retomada`: detectar `finish_reason == 'length'` e refazer **uma vez** sem raciocínio (`reasoning.enabled: false`), quando o orçamento inteiro fica para o JSON. A tentativa cortada é contabilizada em `<action>_truncada` — os tokens foram cobrados, e sumir com eles esconderia o custo real. Duas armadilhas na hora de investigar isso: `langchain-openai` envia **`max_completion_tokens`**, não `max_tokens`, então regras do provedor ancoradas nesse campo nunca se aplicam; e a mensagem de erro precisa separar "JSON malformado" de "saída cortada" — trocar uma pela outra custou um mês de investigação no alvo errado. Em produção o raciocínio saltou de 8–13k (até 11/08/2026) para 35–65k (de 14/08 em diante) sem nenhuma mudança nossa e com o mesmo modelo: qualquer defesa aqui tem de sobreviver a o provedor mudar de comportamento sozinho.
...
| `fap_procuracoes_service`              | Procurações FAP: upsert com diff campo a campo (`TRACKED_FIELDS`), histórico de mudança e os dois builders de e-mail — fonte única do script dedicado, da etapa do cron completo e da rota `POST /fap-panel/sync/procuracoes`. Só `ALERT_FIELDS` (situação e vigência) gera alerta; o resto fica como auditoria. **"Nova" no alerta exige `data_cadastro` recente** (`NOVA_CADASTRO_DIAS`): o portal oscila e às vezes devolve pela primeira vez procurações de anos atrás (09/09/2026: 38 de 2022–2025 junto com a única nova de verdade). Essas vão para o bloco "Apareceram agora no portal" (`descobertas`) e não disparam o e-mail sozinhas |
