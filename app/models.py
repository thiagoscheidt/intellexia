...
class FapContestationClassifierPromptVersion(db.Model):
    """Versões do prompt markdown usado na classificação de contestações FAP."""
    __tablename__ = 'fap_contestation_classifier_prompt_versions'
...
class FapContestationJudgmentReport(db.Model):
    ...
    payroll_mass_records = db.relationship(
        'FapContestationPayrollMass',
        back_populates='report',
        cascade='all, delete-orphan'
    )
    employment_link_records = db.relationship(
...
class FapContestationPayrollMass(db.Model):
    """Payroll mass (Massa Salarial) entries extracted from FAP contestation judgment reports."""
    __tablename__ = 'fap_contestation_payroll_masses'
    __table_args__ = (
        db.UniqueConstraint(
            'law_firm_id', 'report_id', 'employer_cnpj', 'competence',
            name='uq_payroll_mass_law_firm_report_cnpj_competence',
        ),
    )
    ...
    # Competence period (e.g. "11/2023")
    competence = db.Column(db.String(10), nullable=False, index=True)

    # Financial data
    total_remuneration = db.Column(db.Numeric(18, 2))          # Total Remunerações
    first_instance_requested_value = db.Column(db.Numeric(18, 2))   # Valor Massa Salarial Solicitado (1ª instância)
    second_instance_requested_value = db.Column(db.Numeric(18, 2))  # Valor Massa Salarial Solicitado (2ª instância)
    ...
    # Relationships
    law_firm = db.relationship('LawFirm')
    report = db.relationship('FapContestationJudgmentReport', back_populates='payroll_mass_records')
    vigencia = db.relationship('FapVigenciaCnpj')
    source_history = db.relationship(
        'FapContestationPayrollMassSourceHistory',
        back_populates='payroll_mass',
        cascade='all, delete-orphan',
        order_by='FapContestationPayrollMassSourceHistory.updated_at.desc()',
    )
    manual_history = db.relationship(
        'FapContestationPayrollMassManualHistory',
        back_populates='payroll_mass',
        cascade='all, delete-orphan',
        order_by='FapContestationPayrollMassManualHistory.created_at.desc()',
    )

    def __repr__(self):
        return f'<FapContestationPayrollMass {self.employer_cnpj} {self.competence}>'


class FapContestationPayrollMassSourceHistory(db.Model):
    """File audit trail for payroll mass entries."""
    __tablename__ = 'fap_contestation_payroll_mass_source_history'
    __table_args__ = (
        db.UniqueConstraint('payroll_mass_id', 'report_id', name='uq_payroll_mass_source_history_mass_report'),
    )

    id = db.Column(db.Integer, primary_key=True)
    law_firm_id = db.Column(db.Integer, db.ForeignKey('law_firms.id'), nullable=False, index=True)
    payroll_mass_id = db.Column(db.Integer, db.ForeignKey('fap_contestation_payroll_masses.id'), nullable=False, index=True)
    report_id = db.Column(
        db.Integer,
        db.ForeignKey('fap_contestation_judgment_reports.id', ondelete='CASCADE'),
        nullable=False,
    ...
    def __repr__(self):
        return f'<FapContestationPayrollMassSourceHistory mass={self.payroll_mass_id} report={self.report_id}>'


class FapContestationPayrollMassManualHistory(db.Model):
    """User edit history for payroll mass entries."""
    __tablename__ = 'fap_contestation_payroll_mass_manual_history'
    ...
    action = db.Column(db.String(60), nullable=False, default='edit_payroll_mass_first_instance_status', index=True)
    old_first_instance_status = db.Column(db.String(30), index=True)
    new_first_instance_status = db.Column(db.String(30), nullable=False, index=True)
    notes = db.Column(db.Text)
    ...
    def __repr__(self):
        return f'<FapContestationPayrollMassManualHistory mass={self.payroll_mass_id} action={self.action}>'


class FapContestationEmploymentLink(db.Model):
    """Número Médio de Vínculos entries extracted from FAP contestation judgment reports."""
    __tablename__ = 'fap_contestation_employment_links'
    __table_args__ = (
        db.UniqueConstraint(
            'law_firm_id', 'report_id', 'employer_cnpj', 'competence',
            name='uq_employment_link_law_firm_report_cnpj_competence',
        ),
    )
    ...
    # Competence period (e.g. "01/2022")
    competence = db.Column(db.String(10), nullable=False, index=True)

    # Quantity data (integer values)
    quantity = db.Column(db.Integer)                               # Quantidade (original)
    first_instance_requested_quantity = db.Column(db.Integer)     # Número Vínculo Solicitado (1ª instância)
    second_instance_requested_quantity = db.Column(db.Integer)    # Número Vínculo Solicitado (2ª instância)
    ...
    # Relationships
    law_firm = db.relationship('LawFirm')
    report = db.relationship('FapContestationJudgmentReport', back_populates='employment_link_records')
    vigencia = db.relationship('FapVigenciaCnpj')
    source_history = db.relationship(
        'FapContestationEmploymentLinkSourceHistory',
        back_populates='employment_link',
        cascade='all, delete-orphan',
        order_by='FapContestationEmploymentLinkSourceHistory.updated_at.desc()',
    )
    manual_history = db.relationship(
        'FapContestationEmploymentLinkManualHistory',
        back_populates='employment_link',
        cascade='all, delete-orphan',
        order_by='FapContestationEmploymentLinkManualHistory.created_at.desc()',
    )

    def __repr__(self):
        return f'<FapContestationEmploymentLink {self.employer_cnpj} {self.competence}>'


class FapContestationEmploymentLinkSourceHistory(db.Model):
    """File audit trail for employment link entries."""
    __tablename__ = 'fap_contestation_employment_link_source_history'
    __table_args__ = (
        db.UniqueConstraint('employment_link_id', 'report_id', name='uq_el_source_history_link_report'),
    )

    id = db.Column(db.Integer, primary_key=True)
    law_firm_id = db.Column(db.Integer, db.ForeignKey('law_firms.id'), nullable=False, index=True)
    employment_link_id = db.Column(db.Integer, db.ForeignKey('fap_contestation_employment_links.id'), nullable=False, index=True)
    report_id = db.Column(
        db.Integer,
        db.ForeignKey('fap_contestation_judgment_reports.id', ondelete='CASCADE'),
        nullable=False,
    ...
    def __repr__(self):
        return f'<FapContestationEmploymentLinkSourceHistory link={self.employment_link_id} report={self.report_id}>'


class FapContestationEmploymentLinkManualHistory(db.Model):
    """User edit history for employment link entries."""
    __tablename__ = 'fap_contestation_employment_link_manual_history'
    ...
    action = db.Column(db.String(60), nullable=False, default='edit_employment_link_first_instance_status', index=True)
    old_first_instance_status = db.Column(db.String(30), index=True)
    new_first_instance_status = db.Column(db.String(30), nullable=False, index=True)
    notes = db.Column(db.Text)
