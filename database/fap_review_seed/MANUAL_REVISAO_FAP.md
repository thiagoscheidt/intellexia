...
- [ ] **Razão social da Autora — grafia correta** — verificar se a razão social está grafada corretamente, incluindo a pontuação da abreviatura societária. A forma correta exige ponto após "S.A.": "DOHLER **S.A.**", não "DOHLER S.A,".
- [ ] **Razão social da Autora — consistência ao longo de todo o documento** — varrer TODAS as ocorrências do nome da empresa no documento (qualificação, síntese fática, corpo da petição, tabelas, notas de rodapé, pedidos e exemplos) e verificar se a grafia é idêntica em todas elas. Qualquer divergência — letra trocada, suprimida ou acrescida — deve ser reportada como achado CRÍTICO com a localização exata de cada ocorrência incorreta. Exemplo de erro real: empresa grafada como "WHIRLPOOL" na qualificação e como "WHIRPOOL" (sem o segundo "L") em trechos do corpo do texto. A abreviatura societária também deve ser uniforme ("S.A." vs "S/A" vs "SA" são formas distintas — apenas uma deve ser usada).
...
## Regra de suficiência documental por tese

Para cada tese arguida na petição, considera-se suficiente a **demonstração documental completa de
um benefício exemplificativo** — não é obrigatório repetir a mesma prova (telas do FAP, CAT,
extratos, declaração de benefícios etc.) para todos os benefícios enquadrados na mesma tese.

- As exigências de documentos obrigatórios das Seções 1.4 e 3.x devem ser lidas à luz desta regra:
  havendo a cadeia documental completa para ao menos um benefício da tese, **não aponte como "em
  falta"** os mesmos documentos dos demais benefícios da tese.
- A regra não dispensa documentos de natureza única da tese (ex.: sentença/acórdão na tese de
  reforma judicial) — apenas evita a exigência repetida por benefício.
