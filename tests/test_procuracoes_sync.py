...
def teste_sincronizacao_grava_como_sempre(fid):
    print('\n[13] A máscara é só de exibição: a sincronização grava como sempre gravou')
    limpar(fid)
    ...
    check(rec.cnpj_raiz_outorgante == '482840', 'raiz gravada como vem do portal',
          repr(rec.cnpj_raiz_outorgante))
    check(rec.cpf_outorgado == '7488971', 'CPF gravado como vem do portal',
          repr(rec.cpf_outorgado))

    antes = len(historico(fid))
    stats = svc_mod.sync_procuracoes(FakeService([item]), fid)
    check(stats['unchanged'] == 1 and len(historico(fid)) == antes,
          'ressincronizar não gera mudança nem histórico', str(stats))


def teste_filtro_por_cnpj_raiz(fid):
    print('\n[14] Filtro "Nome ou CNPJ raiz" acha com e sem os zeros')
    limpar(fid)
    svc_mod.sync_procuracoes(FakeService([
    ...
    check(rec.cnpj_raiz_outorgante == '482840', 'e o banco segue com a raiz sem os zeros',
          repr(rec.cnpj_raiz_outorgante))


def teste_emails_recebem_raiz_completa(fid):
    print('\n[15] E-mails: a raiz é exibida com os zeros')
    limpar(fid)
...
def main():
    with app.app_context():
        ...
        try:
            ...
            teste_sincronizacao_grava_como_sempre(fid)
